...
# The mv176 Fender-gated bloom-VCA live-sweep, retired unused at mv179 before
# the user ever heard it. ⚠️ This frontier was CLOSED 2026-08-04 (third
# attempt, "let's abandon this feature") — re-read pa-compression-fender in
# full before extending beyond this ears-only session. Defaults = the stock
# Fender values (depth 0.15 / release 13 ms) = bit-identical. Pure tail append.
...
def emit_icon():
    ...
    return ('<div class="mod-pedal mod-pedal-guitaramp-hexforge{{{cns}}} ">\n'
        ...
        # ── LAB strip (TEMPORARY, Fender PA-compression ears session): the two
        # bloom-VCA sweep knobs, bottom-left; this mod-ui build has no
        # generic-port view so lab ports need on-panel widgets.
        '  <div class="hf-lab" role="group" aria-label="Tuning lab (temporary)">'
        '<span class="hf-lab-t">LAB</span>'
        + render_ctrl(CTRL_BY_SYM["dbg_pacomp"])
        + render_ctrl(CTRL_BY_SYM["dbg_parel"])
        + '</div>\n'
