...
# ── EVH capture-fit LAB, RESTORED BY USER REQUEST (2026-08-22, TEMPORARY):
# the mv177-era dbg_evhfit blend, back for a re-tune session — the EVH has
# since gained the correct 6L6 tube (mv186) and the user now monitors through
# the FRFR voice, so the ear-chosen 0.8875 may want revisiting. DEFAULT =
# 0.8875 = the baked blend = bit-identical at rest; 0 = no correction EQ.
# Tail port; bake + retire after the session.
ctrl.append(mkport("DBG_EVHFIT", "dbg_evhfit", "EVH Fit Blend", "f", 0, 1, 0.8875, None, "EVHFit"))
...
def emit_icon():
    ...
    return ('<div class="mod-pedal mod-pedal-guitaramp-hexforge{{{cns}}} ">\n'
        ...
        '</div>\n'
        '  </div>\n'
        # ── LAB strip (TEMPORARY, EVH fit re-tune session): one blend knob,
        # bottom-left (the spot freed when OUT moved into the ADVANCED popout).
        '  <div class="hf-lab" role="group" aria-label="Tuning lab (temporary)">'
        '<span class="hf-lab-t">LAB</span>'
        + render_ctrl(CTRL_BY_SYM["dbg_evhfit"])
        + '</div>\n'
