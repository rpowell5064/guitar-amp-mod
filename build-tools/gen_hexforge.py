...
DR = [
    ("model",  "Model",  "e", 0, 11, 0, [("Green Man",0),("New Dawn",1),("Dear Rodent Boy",2),("Neural (NAM)",3),("Grunge DS",4),("Gilded Horse",5),("Super Nova",6),("Preamp 250",7),("Echo Primer",8),("Tube Chauffeur",9),("Helsinki Grind",10),("Treble Ranger",11)]),
...
AMP = [
    ("model",         "Model",        "e", 0, 15, 1, [("Clean Meanie",0),("Crunchy McCrunchFace",1),("Gainzilla",2),("Doom Daddy",3),("Tangerang",4),("Neural (NAM)",5),("Beardo BE",6),("Hi-Volt",7),("Chime Thirty",8),("Backline Plus",9),("Plexiglass",10),("Cali V",11),("Diamond Plate",12),("Tremont 15",13),("Blue Liner",14),("Citrus 200",15)]),
...
MD = [
    ("type",  "Type",  "e", 0, 8, 0, [("Lush-2",0),("Uni-Verse",1),("Phaser",2),("Flanger",3),("Tremolo",4),("Rotary",5),("Nevermind Chorus",6),("Seasick Vibe",7),("Script Phaser",8)]),
...
# active}. Migrated v23.
EQ_POS_DEFAULT = 6
# max 24, NOT 13 (2026-08-23): every renumber can push EQ past 13 now that the
# X2 clones park at 15-24; mod-host clamped the write, the echo mismatched the
# UI's data-pos, and the modgui rewrote every pos port in response -> infinite
# rewrite storm (all clients flashing/reordering). Keep ALL pos maxima == the
# movable-slot count.
ctrl.append(mkport("EQ_POS",    "eq_pos",    "EQ Position", "e", 1, 24, EQ_POS_DEFAULT,
                   [(str(k), k) for k in range(1, 25)], "Slot", hidden=True))
...
# 2026-09-21: option 2 = Space, the geometric (image-source) room — no new port.
ctrl.append(mkport("CAB_ROOMDENSE", "cab_roomdense", "Cab Room Density", "e", 0, 2, 0,
    [("Classic",0),("Dense",1),("Space",2)], "Room Density"))
...
# 2026-09-21: option 3 = Physical, the state-based large-signal driver (SpeakerModel.h).
ctrl.append(mkport("CAB_SPKDRIVE", "cab_spkdrive", "Cab Speaker Drive", "e", 0, 3, 0,
    [("Off",0),("Subtle",1),("Full",2),("Physical",3)], "Speaker Drive"))
...
# family. Migrated v35.
RB = [
    ("amp",     "Amp",      "e", 0, 15, 1, [("Clean Meanie",0),("Crunchy McCrunchFace",1),("Gainzilla",2),("Doom Daddy",3),("Tangerang",4),("Neural",5),("Beardo BE",6),("Hi-Volt",7),("Chime Thirty",8),("Backline Plus",9),("Plexiglass",10),("Cali V",11),("Diamond Plate",12),("Tremont 15",13),("Blue Liner",14),("Citrus 200",15)]),
    ("gain",    "Gain",     "f", 0, 1, 0.5, None),
    ("bass",    "Bass",     "f", 0, 1, 0.5, None),
    ("mid",     "Mid",      "f", 0, 1, 0.5, None),
    ("treble",  "Treble",   "f", 0, 1, 0.5, None),
...
# ── Amp 2 NAM trims (2026-07-30, user request: "shouldn't these mirror their
# counterparts?"): the B side gains its own Neural slot (#amp2nam path param) --
# these are its Gain/Level trims, cloned from the A-side amp_nam_gain/amp_nam_vol
# defs so ranges can never drift. Migrated v39.
...
# ── Plexi Variac (2026-07-30, user: "open up van halen 1984 sounds"): the EVH
# brown-sound trick -- the 1959 at ~90 V. Toggle, default 0 = stock/bit-identical.
...
# ── Rig B blend low cut (2026-08-21): HP on the rig-B layer's mix-in. The
# dual-rig clean blends partially CANCEL rig A (measured polarity), and the
# clean layer's own dynamics pump the cancellation depth in the low-mids — the
# Periphery "woosh", isolated by build-tools/woosh2.cpp (rigB-off collapsed
# the low-band flutter; HF/DNR/gate/spkdrive all measured innocent). Cutting
...
# ── Helsinki Grind component build (2026-09-12): the Darkglass B7K's own panel —
# BASS / LO MIDS / HI MIDS / TREBLE (0.5 = flat), the 3-way GRUNT (Fat/Raw/Thin)
# and ATTACK (Flat/Boost/Cut) switches — shown only while drive model 10 is
# selected (c-dr-b7k); Tone is hidden there (the Grunt switch replaced it).
# Drive 2 gets its explicit twins (Drive 2 is not an X2 clone). Migration gaps
# MUST default to noon (0.5) and Raw (1) / Flat (0). Migrated v49.
_B7K_GRUNT  = [("Fat",0),("Raw",1),("Thin",2)]
_B7K_ATTACK = [("Flat",0),("Boost",1),("Cut",2)]
for _pfx, _ttl in (("DR", "Drive"), ("DR2", "Drive 2")):
    _lo = _pfx.lower()
    ctrl.append(mkport(_pfx + "_B7K_BASS",   _lo + "_b7k_bass",   _ttl + " B7K Bass",    "f", 0, 1, 0.5, None, "Bass"))
    ctrl.append(mkport(_pfx + "_B7K_LOMID",  _lo + "_b7k_lomid",  _ttl + " B7K Lo Mids", "f", 0, 1, 0.5, None, "Lo Mids"))
    ctrl.append(mkport(_pfx + "_B7K_HIMID",  _lo + "_b7k_himid",  _ttl + " B7K Hi Mids", "f", 0, 1, 0.5, None, "Hi Mids"))
    ctrl.append(mkport(_pfx + "_B7K_TREBLE", _lo + "_b7k_treble", _ttl + " B7K Treble",  "f", 0, 1, 0.5, None, "Treble"))
    ctrl.append(mkport(_pfx + "_B7K_GRUNT",  _lo + "_b7k_grunt",  _ttl + " B7K Grunt",   "e", 0, 2, 1, _B7K_GRUNT, "Grunt"))
    ctrl.append(mkport(_pfx + "_B7K_ATTACK", _lo + "_b7k_attack", _ttl + " B7K Attack",  "e", 0, 2, 0, _B7K_ATTACK, "Attack"))

# ── Cab Mic 2 (Phase 3, 2026-09-21): a second virtual microphone on the same
# convolution — Type (Off / the IR's own dynamic-57 class / dynamic-421 / ribbon /
# condenser, curves RELATIVE to the mic the IR was made with), its own Pos/Dist
# morphs, Blend against the primary, Align (coherent, the old Studio convention)
# and Polarity. Off = every legacy path bit-identical; Blend gap-fills 0.35.
# Cab 1 then Rig B's cab. Migrated v50.
_MIC2_TYPE = [("Off",0),("Dynamic 57",1),("Dynamic 421",2),("Ribbon",3),("Condenser",4)]
for _pfx, _sym, _ttl in (("CAB_", "cab_", "Cab"), ("RB_CAB", "rb_cab", "Rig B Cab")):
    ctrl.append(mkport(_pfx + "MIC2TYPE",  _sym + "mic2type",  _ttl + " Mic 2 Type",     "e", 0, 4, 0,    _MIC2_TYPE, "Mic 2"))
    ctrl.append(mkport(_pfx + "MIC2POS",   _sym + "mic2pos",   _ttl + " Mic 2 Pos",      "f", 0, 1, 0,    None, "Pos 2"))
    ctrl.append(mkport(_pfx + "MIC2DIST",  _sym + "mic2dist",  _ttl + " Mic 2 Dist",     "f", 0, 1, 0,    None, "Dist 2"))
    ctrl.append(mkport(_pfx + "MIC2LVL",   _sym + "mic2lvl",   _ttl + " Mic 2 Blend",    "f", 0, 1, 0.35, None, "Blend"))
    ctrl.append(mkport(_pfx + "MIC2ALIGN", _sym + "mic2align", _ttl + " Mic 2 Align",    "t", 0, 1, 0,    None, "Align"))
    ctrl.append(mkport(_pfx + "MIC2POL",   _sym + "mic2pol",   _ttl + " Mic 2 Polarity", "t", 0, 1, 0,    None, "Polarity"))
...
# (The 2026-08-19 dbg_* tuning-lab tail ports lived here between mv176 and
# mv178; retired at mv179 after the EVH fit was baked at the ear-chosen blend
...
# (The 2026-08-20 dbg_hgfit HG-round-2 LAB port lived here between mv180 and
# mv181; retired after the user approved the JCM800 + Tremont 15 candidates at
# blend 0.72 — baked into AmpDefaults rows 1/8 + the Plexi/MarkV row-10 split.)
# (The 2026-08-20 dbg_rectofit Recto-voicing LAB port lived here for mv182
# only; retired at mv183 after the user baked the trusted-ladder fit at blend
# 1.0 — RectoCaptureFit.h + the model's mode-7 satDrive/tightHP row.)
# (The 2026-08-20 restored Fender PA-compression lab ports lived here for
# mv184 only. VERDICT: the user swept it by ear and chose STOCK 0.15/13 ms —
# nothing baked; the PA-compression frontier is closed with an EARS verdict.)
# ── Output Voice (2026-08-21, PERMANENT): global per-listening-rig layer, the
# auto-cal pattern — tail port, never preset-captured, presets untouched.
# OFF = Headphones/Studio (bit-identical stock). ON = FRFR Room (tuned for the
# user's Fender FRFR-10): de-close-mic EQ post-mono-sum, pre-Output, and the
...
# Output polarity (2026-09-05, user request). A real tube amp has an ODD number of
# inverting gain stages, so its speaker moves opposite the pick attack; the reference rig
# models that and ships output-inverted, which is why an the reference rig reads "phase
# inverted" by default. ON by default here to match, so this rig and the reference rig sum
# correctly when compared or blended. NOTE this is NOT the same lever as the
# Input Trim phase switch: inverting at the INPUT changes which half of the wave
# hits our asymmetric clipping first (a tone change), while inverting at the
# OUTPUT is a pure polarity flip — inaudible alone, and only matters when the
# signal is summed with another source. Applied dead last, after the limiter.
ctrl.append(mkport("OUT_PHASE", "out_phase", "Output Phase Invert", "t", 0, 1, 1, None, "Phase"))

# EVH 5150 III COMPONENT MODEL toggle (2026-09-09, experiment): swaps the EVH
# amp (model 2) between the shipped capture-fitted EVH5150Model and the
# schematic-exact EVH5150ComponentModel (Fender drawing 0079092000: full
...
def emit_ttl():
    ...
    L.append("")
    L.append("# Cab 2 (Rig B) user IR parameter -- mirrors #irfile.")
    L.append("<%s#ir2file>" % URI)
    L.append("    a lv2:Parameter ;")
    L.append('    rdfs:label "Cab 2 IR" ;')
    L.append("    rdfs:range atom:Path ;")
    L.append('    mod:fileTypes "cabsim,ir,wav,audio" .')
    L.append("")
    ...
    L.append("#   rigs     : UI<->plugin — the user's saved cab rigs (JSON), kept in State + mirrored to disk (2026-09-22; sorts last: rigs=11).")
    for frag, lbl in (("ps_name", "Preset Name"), ("ps_index", "Preset Index"), ("ps_apply", "Preset Apply"), ("meters", "Level Meters"), ("rigs", "Saved Rigs")):
        L.append("<%s#%s>" % (URI, frag))
        L.append("    a lv2:Parameter ;")
        L.append('    rdfs:label "%s" ;' % lbl)
    ...
    # PatchStorage listing content: the patchstorage-lv2-uploader publishes the
    ...
    L.append('    rdfs:comment """' + """Your entire guitar AND bass rig in one plugin — 14 amps (incl. a classic all-tube bass head), 11 drives, 4 fuzzes, mod/delay/3-type reverb/EQ/wah/octave, cab mic + room, 60+ song-inspired presets, footswitch recall, and a full node-chain UI.
...
- DRIVE: ELEVEN overdrive/boost/distortion voicings, from the green mid-hump legend and the mythical gold horse to the starved-tube Tube Chauffeur, the germanium Treble Ranger and the Helsinki Grind parallel-blend BASS preamp.
...
On 64-bit/desktop targets the amp and drive slots load your own .nam captures (the cab slot is pure IR/convolution); on 32-bit (patchbox-os-arm32) NAM is hidden and the algorithmic models are used. The heaviest plugin in the suite — best on Pi 4/5 and desktop. Free and open source (GPL-3).""" + '""" ;')
    ...
    L.append("    lv2:microVersion 267 ;")   # 267: the rename box never sends a blank name. 266: a user .wav on a cab panel shows the basics only (rigs + MIC & ROOM are for the built-in cabs). 265: a recall re-reads its rig once the IR path lands. 264: a user IR keeps MIC & ROOM and the rigs (a rig applies its placement to the user cab; recognised on it). 263: Studio Pair rig row was one value short (Room Density missing -> Mic 2 fell to Off, blend 1, polarity NaN); fixed + rig rows are length-checked. 262: micKeys helper restored (the rigs patch had replaced the block holding it). 261: saved rigs are seeded from the start parameters (mod-ui hands the last pushed value at GUI open; change events only follow live pushes). 259: the panel pulls the saved rigs with patch_get on load. 258: rigs URI literal (the script read a not-yet-declared var and mod-ui disabled it). 257: USER RIGS — save the current cab stage as a named rig from the Rig list (MY RIGS section, tap-twice delete); stored on the device (#rigs string param in State + hexforge-rigs.json beside the preset backup), so they travel with the board and survive delete/re-add. 256: mic pad body stacks tabs / pad / Mic 2 controls in a column (cache bump). 255: MIC 1 / MIC 2 tabs on the mic pad — one draggable pad for both mics, Mic 2\'s Type / Blend / Align / Polarity under the pad on its tab (the separate MIC 2 knob group is gone; same ports). 254: rigs are named by the cab the IR picker shows (Factory 4x12 - Tight 57 ...) and set the cab through the host's patch_set instead of clicking the picker (which refuses clicks while its widget is not flagged enabled). 253: rig label resets to Custom on preset recall (cache bump). 252: RIGS — a Rig selector beside each cab IR picker: ten factory rigs (cab + speaker drive + mic placement + Mic 2 + room + voice + cuts) applied through the existing per-preset ports; no port, no preset moves until a rig is chosen. 251: DYNAMIC LOAD lab toggle beside Component Model - the component power sections drive the large-signal speaker model as their load (current-driven) instead of two static impedance filters; global tail port amp_dynload, default off = bit-identical. 250: ROOM SPACE — the Cab Room Density selector gains a geometric room: image-source early reflections in a shoebox (cab by a wall, a spaced stereo pair a few metres back), Room Size scales the box and pulls the pair back, late field from a Sabine RT60; Classic/Dense untouched. 249: CAB MIC 2 — a second virtual microphone on the Cabinet block (Cab 1 + Rig B's cab): Type (Off / Dynamic 57 = the IR\'s own mic / Dynamic 421 / Ribbon / Condenser), its own Pos + Dist, Blend, Align (coherent) and Polarity; twelve per-preset ports, blob v50, Off = bit-identical. 248: Speaker Drive gains a PHYSICAL option - a state-based large-signal driver (SpeakerModel.h: excursion-dependent force factor/stiffness/inductance, back-EMF, voice-coil heating) replacing the envelope heuristics when selected; calibrated in volts from the component power sections; small-signal transparent (the driver's own linear response is divided back out), so Off/Subtle/Full and every preset are untouched. 247: KNOB INDICATOR FIX - the generated knob sprites were rotated 180 degrees from the stock MOD convention (frame 50 aimed straight DOWN instead of straight UP), so every knob pointer on the amp panels showed the wrong value. make_knobs.py regenerated, all six families. 246: preset menu widened 560 -> 680px so Backup / Restore no longer hang off the right (the header row was already fully compressed -- search at its 90px min-width, spacer at 0 -- and still overflowed by 67px); the row can now wrap as a safety net. Themed scrollbar on the preset list and the long dropdowns actually applies now: it was coloured with var(--acc), which is only ever defined inline on the per-block tiles, so in the toolbar-anchored menu the variable was undefined and the declaration dropped to the browser default. CSS only: no port moved, no preset changed, no DSP touched. 245: AMP HEAD ART — the head is a cabinet: per-model covering material (16 procedurally generated tolex tiles: pebbled vinyl, basketweave, elephant hide, carbon, lacquered linen), rolled top edge, side returns, corner protectors, and a faceplate set into it. Per-model faceplates for all 16 amps (brushed / polished / anodised / enamel / carbon, lit by a shared studio rig) and per-family KNOBS (six 101-frame sprites: skirted-with-chrome-cap, top-hat with brass collar, ivory pointer, machined-aluminium skirt, copper collar, graphite with a cyan indicator). The faceplate PIPING is a neon tube that cycles red -> blue -> purple -> green and casts its light on the panel, with a matching strip where the valves stand; each panel seeds a random start so no two run in step. New GUITAR / BASS filter on the Amp, Amp 2 and preset menu: hides list entries only (never a stored value), bass = amps 14/15, cabs 7-10 + @bass* IRs, preset bank 19. Screws removed. Markup + art only: no port moved, no preset changed, no DSP touched. 241: CITRUS 200 (amp model 15)   # 241: CITRUS 200 (amp model 15) - the suite's SECOND bass amp: an Orange AD200B Mark III built schematic-exact from the Mark III preamp/power-amp drawings (two ECC83 gain stages, the FMV stack solved as a network, an ECC83 long-tail pair with the global loop into its tail, 4x 6550, 200 W). No fitted twin, so it carries its own power section unconditionally: no Presence knob, no Power Amp tab. 240: AMP PANEL RULE — a knob is shown only if the REAL amp has it (Deluxe Reverb: no Middle/Presence/Master; AC30 Top Boost: no Middle/Master, its Presence IS the Cut; Plexi: non-master; Rockerverb/Backstage/MT15/SVT: no Presence; SVT: no Master; Rockerverb CLEAN also drops Middle + channel Volume). Visibility only — the stored value still feeds the model, so no preset changes. Applies to Amp, Amp 2 and the standalone Amp plugin. 239: Helsinki Grind COMPONENT build (Darkglass B7K from its circuit drawing: J201 follower + fixed voicing, TL072 drive stage with the 3-way GRUNT coupling, 4049 CMOS Microtubes clipper, the post low-passes, Blend, the passive James bass/treble stack + two active mid bands) with the pedal's own panel — Bass / Lo Mids / Hi Mids / Treble knobs + Grunt / Attack switches on both Drive blocks (12 per-preset ports, blob v49); Tone hidden on model 10. 238: Component Model toggle shown on the Blue Liner (Ampeg SVT component build from the 591719/591720 factory drawings: twin-T Ultra-Lo, bridged-pot tone module, R35 preamp loop, inductor mid, D1/D2 input limiter, cathodyne + 12BH7 drivers, 6550 sextet with the loop into V1-A). 237: Component Model toggle shown on the Chime Thirty (Vox AC30 Top Boost component build: 1971 VSL top-boost sheet + OS/065 factory voltages, cathode-biased EL84 quad). 236: Component Model toggle shown on the Rockerverb (Orange Rockerverb 50 component build from the ORA-CD204/CD206 factory drawings, both channels). 235: Component Model toggle shown on the Diamond Plate (Mesa Dual Rectifier Rev F component build, two-channel LDR states mapped onto the 8-way mode selector). 234: Component Model toggle shown on the Cali V (Mesa Mark V channel-3 component build). 233: Component Model toggle generalised beyond the EVH (new c-amp-comp visibility class) + component JCM800/Friedman. 232: 5150 Component Model lab toggle (amp_evhcomp, global tail): schematic-exact EVH from Fender drawing 0079092000 — component preamp + LTP/2x6L6GC/NFB/screen-sag/OT-flux power section, hardware-verified vs the user's Axe-FX probe session (Red specESR 16-22); shared PA bypassed while ON.   # 231: noise pass round 2 (user's explicit trade) — EVH PA presence 0.63->0.25 (~5 dB quieter for ~0.4 dB of reference match; now QUIETER than before the re-voice) and Vox stage span 1.8->1.3 (~5 dB for 0.76 dB). The recurring culprit across all three amps was the same shape: a gain or treble boost applied LATE in the chain, which lifts amplified noise more than signal. 230: NOISE PASS (user: "more noise even while playing") — the re-voiced amps amplify the rig floor; measured the real input floor off the device (-60.1 dBFS broadband hiss, hum 24 dB below it — the old "-45 dBFS/89%% hum" note was stale). Removed two pure noise taxes that also measured WORSE against the hardware: the JCM800 post-clip HF opens (+6/+5 dB -> +1.5/+1.0) and the Vox +3 dB input drive (-> 0). 229: user dial-ins baked (rev 141) + the reference rig amp-fix audit fixes: Cali V Ch3 remap no longer corrupts user presets on rev bumps (was a non-idempotent sweep over ALL slots), EVH/JCM master taper survives reset(), Vox/Recto/MarkV fit filters now cleared on reset, softLimit knee divide guarded, Recto duplicate fit_tighthp handler removed. 228: TREBLE RANGER — drive model 11, the Dallas Rangemaster germanium treble booster (Clapton Beano / Brian May / early Iommi): one OC44 common-emitter stage solved Ebers-Moll with a Newton-Raphson load line, its 5 nF input cap keeping the lows OUT of the amp VALIDATED against R.G. Keen's measured original: bench response within 0.62 dB mean / 0.84 dB worst across 100 Hz-20 kHz, unity gain at 89 Hz (his ~80 Hz = the open low E), H2-dominant harmonic ladder matching a measured-parameter circuit sim through the playing range. In use the pickup loading turns the bench shelf into the real band-pass, peaking +19 dB at 1.2 kHz. Tone = the input-cap RANGE mod (5 nF treble -> 22 nF mid -> 100 nF full-range boost). Model-only addition: no port moved, no preset changed. 227: rev-139 research rebuild curated by ear -- the user kept Ghost (Meliora + Impera/Skeleta banks), the BTBAM Spectrum pair, Numb Sustain and Desert Robot; the other 41 presets rolled back to their pre-rebuild definitions. 226: FROM-SCRATCH factory preset rebuild (rev 139) — 53 presets re-authored from per-song rig research (8-agent web fan-out: producer/engineer interviews, gilmourish, SoS, thekingofgear, Rasmussen notes...) on the reference-fitted amps; golden-leveled to the Bank-1 anchors; bank 1 / NIN B-D / Plexi / user builds untouched. 225: Vox rail now RAMPS with the gain knob (full +18 by 0.30; the flat rail put the amp floor at >=20% THD so clean/edge presets could not exist — user ears caught it; the reference rig g25 match IMPROVES 27.2->20.7) + the five Vox dirt presets rematched to their old drive points at honest TB dials. 224: overnight drive-point restoration — factory presets on the re-voiced amps (EVH/JCM/Recto-Modern/Vox) re-dialed so each keeps its measured THD@1k drive intent on the now-real amps; Vox cleans floored at a real 0.12 dial (ears pending); Cardinal pair set to the user's stated real gain 0.55 (their 0.15 compensated the old 2x-hot Ch3 law). 223: Rockerverb dirty channel: reference RV50 probe verdict = TONE ALREADY EXCELLENT (grid mean 12.4%, best in the suite — the exact tonestack + h2/evens work vindicated by hardware); only the gain-knob loudness law shipped (PA row trackLo 1.71/trackHi 0.13 — the real dial drops 10 dB more below noon). 222: Vox AC30 (Chime Thirty) re-voiced to the user's 7-take reference Class-A 30W top-boost probe (drive extension span x1.8 gain-coupled + rail +18 — our TB was too polite to reach the real amp's early crank; AC30 honk restored pk150 -8/pk1800 -7/hs4k -5; specESR mean 48.8->26.6; silence-probe CLEAN -300 dBFS dimed). 221: Cali V Mark IV mode re-voiced to the user's 8-take reference USA-lead probe (lows out of the clip inHP 75->160 + interLP 15k->7k, post 125 Hz hump +10 / mids -6 / top shelf -8; specESR mean 40.9->18.7). 220: Cali V Ch3 (IIC+/IV/Extreme) knob laws re-lawed to the reference IIC+ probe (gain max(0.6g,g-0.1), treble 0.24t/0.12+0.8(t-0.5) — our knobs ran ~2x hot; noon 38.7->23.8, g25 50->11.5, mid_min 55->29); ALL saved presets sound-preserved via seed rev-133 + blob v46 inverse remap. 219: Recto VINTAGE modes voiced too (VB: pk1200 -3 / hs7500 +6 / pk200 -3, modes 3+6 only; every the reference rig Vintage take improved or held, redvin noon 18.2->17.0). 218: Recto (Diamond Plate) MODERN modes re-voiced to the user's 22-take reference rectifier probe grids (worst-in-suite 39-45% -> ~31: 125 Hz chug peak, tightHP 260->60 = lows into the clips, sag x0.1, passive-presence + master-drive laws, 82 Hz notch off; Vintage modes tone-untouched + makeup fixed; RectoCaptureFit RETIRED; Recto PA row trackLo -0.63). 217: Friedman BE/HBE post-PA gain-level tracking (reference-measured: the gain knob now spans the real amp's loudness range, trackLo .95/trackHi .12; PA also gains dormant duck-and-swell supply machinery, rows 0 = inert). Model itself validated ALREADY-matched vs 28 reference takes (BE 18.2%/HBE 22.1% mean) — tonestack RC, mid taper, LF voicing all measured correct as-was. 216: JCM800 re-voiced to the user's 12-take reference 2203-high probe (railed level-invariant core rail +18 dB, open top/bottom to the reference rig High input, tighter LF into the clip; specESR mean 25.5->19.7). SIR #34 mod unchanged; user-preserved Jungle Sleaze/Crunch pending A/B. 215: EVH 5150 re-voiced to the user's 24-take reference probe session (railed level-invariant core +15 dB, rounder knee, measured voicing EQ, REAL master taper ^2.4 post-limiter; EvhCaptureFit/evhLab stack RETIRED; specESR mean 32->21.8 over the full knob grid, both channels). 214: LV2-state blob stamp fixed to 45 (v45-layout saves were stamped 44 since 2026-08-28 -> mis-migrated on load; loader disambiguates by param count). An Amp Quality Hi-Fi 8x state (214-215) was built, ear-tested and REVERTED same-day — user verdict: Standard 4x sounds fine (measurements in tools/amp_alias 8x column). 213: Blue Liner (Ampeg SVT) BASS amp (model 14, Ultra-Lo/Hi + 3-way mid, 6550 power tube, blob v45) + four bass cabs (@bass810/@bass410h/@bass210/@bass115, rb_cab 7-10 after the frozen No Cab 6). 212: PatchStorage listing description baked into the TTL (plugin-level rdfs:comment). 211: pos-echo rewrite storm fixed (eq_pos max 13->24 + the modgui never writes pos ports from change echoes). 210: OUT jack LED = plugin on/off (toolbar powerswitch retired), IN jack LED = master mute (tuner_mute decoupled from tuner_on); LEDs act like tile dots (lit/dark + hover/pressed states, hit halos); ADD button sized/aligned to the end caps. 209: IN/OUT dot back above the label, warmer slate-blue chip; block toggle LEDs 14px + invisible ~30px hit halo. 208: IN/OUT squares vertically centred on the block row; label brighter/heavier (13px 900, lighter gradient stops). 207: IN/OUT squares bigger (12px label on top, jack dot below); helix drift back to the original subtle pace. 206: IN/OUT = one larger rounded square containing the jack dot + gradient label; helix particle flow sped up so signal visibly travels IN->OUT. 205: drop slot more noticeable (accent fill, full-opacity 2px dashes, breathing glow). 204: grab-without-move lift race fixed (tiles no longer vanish); IN/OUT = gradient-text chips on dark rounded boxes + jack dots; ADV icon is a real cog. 203: drag polish — dragged tile collapses out of the row (ghost IS the block, no doubled tile), ghost carries the block accent + name, IN/OUT end caps redesigned as quiet jack dots on the signal line. 202: ghost dead-zone fixed (pointer-events:none sent over-ghost hovers to the container = bounce + drop-at-end); FRFR knob reveal now observes the switch widget class. 201: ghost tile animates open/closed (smooth row parting; collapsing clone at the old slot on moves). 200: insertion preview v2 — dashed GHOST TILE placeholder (row parts to make room; cursor-dead-zone placement, no jitter). 199: drag-and-drop insertion preview (glowing landing-slot bar, gap/tail drops, palette drag-to-place, drag-to-palette removes). 198: Calibrate back bottom-right; FRFR knobs show only when the voice is ON; ADVANCED and the preset menu are mutually exclusive. 197: Tube Chauffeur (Butler Tube Driver, drive model 9 — starved-12AX7 warm OD/clean boost, the Gilmour staple) + Numb Sustain restage + Desert Robot gain trim. 196: EVH lab CANCELED unused (bake 0.8875 kept); ADVANCED = FLOW strip under the toolbar (no longer covers controls); Doubler switch label overlap fixed. 195: EVH fit LAB restored (dbg_evhfit blend, default 0.8875 = the bake = bit-identical; TEMPORARY re-tune session on the 6L6/FRFR rig). 194: rb_locut — HP on the rig-B blend layer (blob v44; kills the dual-rig low-mid cancellation pump = the residual Periphery woosh; 0 = off). 193: monoRoom — with the output mono-summed the cab room uses ONE bank (the two decorrelated banks folded into a flanger-like comb on one-speaker rigs; the Periphery woosh). 192: Green Man (TS-808) gain-floor fix (x11.85 mid-band floor at drive 0 = the real boost trick; capture anchor d=0.5 exact). 191: ADVANCED popout enlarged (wrap rows, FRFR knobs always visible, row labels). 190: ADVANCED popout (Auto-Limit/Mono/Doubler/FRFR voice/Calibrate off the toolbar; ADV gear toggles), tuner button back in the toolbar. 189: FRFR voice levers promoted to PERMANENT user knobs (fv_locut/prox/pres/fizz, defaults = the in-room reference bake; knobs show only while OUT is on). 188: FRFR voice BAKED at the user's in-room values (locut 100.75 / prox 1.815 / pres 3.225 / fizz 2.70), dbg_fv_* knobs retired, out_voice switch permanent (OUT corner strip). 187: Output Voice FRFR global layer (out_voice PERMANENT + 4 TEMPORARY dbg_fv_* LAB knobs for the FRFR-10 ears session; presets untouched, off = bit-identical). 186: tube-correctness audit — Fender 6V6 (new TubeType), EVH 6L6, Sunn KT88(inert); manual Power Tube dropdown gains 6V6. 185: Fender PA lab retired — user swept it by ear and chose STOCK 0.15/13 ms; nothing baked, PA-compression frontier closed with an ears verdict. 184: Fender PA-compression LAB RESTORED by user request (TEMPORARY). 183: Recto CH3-Modern capture-fit BAKED at user blend 1.0 (RectoCaptureFit.h + mode-7 satDrive 1.84/tightHP 200; loudness-parity makeup), lab retired. 182: Recto voicing LAB (TEMPORARY). 181: HG round 2 BAKED at user blend 0.72 (JCM800 padrive 1.72 + knee 0.36; Tremont tilt 2.88/6.84; Plexi/MarkV frozen on row 10), lab retired. 180: HG round 2 LAB. 179: EVH capture-fit BAKED. 178: LAB strip. 177: PA lab UNIVERSAL. 176: PA-compression LAB. 175: AUTO-CALIBRATE wizard (3-phase measurement -> global cal_trim_offs/cal_floor_offs layer, 5 tail ports, #cal notify) + post-Apply APPLIED feedback (near-zero recs on the reference rig are the SUCCESS case and now say so). 174: per-amp PA drive/makeup now on the main rig (EVH/Rockerverb/Vox un-over-driven; was Rig-B-only). 173: REVERTED the Fender PA re-voice (172) -- didn't sound right. Back to the pre-revoice PA. 171: Script Phaser mod type 8 (no-feedback script-era P90) + md type clamp fix (Seasick Vibe silently ran as Nevermind Chorus in the Forge). 170: SIR #34 mod toggle on the JCM800, both amps (blob v43). 169: EP-3 Echo delay type (JFET preamp + Age, dl_age/dl2_age, blob v42) + Echo Primer drive model 8. 168: Plexi Variac (brown sound) on both amps. 167: quick wins (FF dead Tone hidden, fuzz/nail Eco, Drive 2 Neural). 166: Voicing/Channel tab label + Friedman defaults HBE. 165: Cab 2 unified IR picker (matches Cab 1) + quiet REMOVE button. 164: Amp 2 NAM + Cab 2 user IR + mirrored cab names (blob v39). 163: Amp2/Cab2 CPU badges + dashed-off strips. 162: Amp2/Cab2 call-to-action strips (+ prefix, filled-when-on). 161: a 2 of every effect (X2 clones, palette-gated). 160: Amp 2/Cab 2 FULL amp/cab UIs + in-tile strips (2026-07-30). 159: standalone panels   # 142: tremolo Shape selector conditional on Type=Tremolo (2026-07-29). 141: search clear ×. 140: preset-menu search box (2026-07-25)
    L.append("")
    ...
    L.append("                   <%s#ps_name> , <%s#ps_index> , <%s#ps_apply> , <%s#meters> , <%s#rigs> ;" % (URI, URI, URI, URI, URI))
    L.append("")
    # audio ports
    L.append("    lv2:port [")
...
COND = {
    ...
    # RULE (2026-09-13, user: "only show the controls that amps have — exact to the
    # original"): a knob is on the panel only if the REAL amp has it. A Deluxe Reverb
    # has no Middle, Presence or Master; an AC30 Top Boost has no Middle or Master (its
    # Presence IS the Cut control); a 1959 Plexi is a non-master amp; a Rockerverb, an
    # MT15, a Backstage Plus and an SVT have no Presence; an SVT has no Master either;
    # and a Rockerverb's CLEAN channel loses its Middle and its channel Volume too.
    # Visibility only — the stored value still feeds the model, so presets are untouched.
    "amp_mid":"c-amp-mid", "amp_presence":"c-amp-pres", "amp_master":"c-amp-mast",
    "amp_resonance":"c-amp-reso",        # EVH(2)
    "amp_evhcomp":"c-amp-comp",          # shown for any amp that HAS a component model
    "amp_dynload":"c-amp-comp",          # Phase 5: rides with the Component toggle
    ...
    "dr2_drive":"c-dr-alg", "dr2_tone":"c-dr-tone", "dr2_level":"c-dr-alg",
    "dr2_nam_gain":"c-dr-nam", "dr2_nam_vol":"c-dr-nam",
    "dr_drive":"c-dr-alg", "dr_tone":"c-dr-tone", "dr_level":"c-dr-alg",
    # Helsinki Grind (model 10): its own 4-band EQ + Grunt/Attack switches; Tone hides there
    "dr_b7k_bass":"c-dr-b7k", "dr_b7k_lomid":"c-dr-b7k", "dr_b7k_himid":"c-dr-b7k", "dr_b7k_treble":"c-dr-b7k",
    "dr_b7k_grunt":"c-dr-b7k", "dr_b7k_attack":"c-dr-b7k",
    "dr2_b7k_bass":"c-dr-b7k", "dr2_b7k_lomid":"c-dr-b7k", "dr2_b7k_himid":"c-dr-b7k", "dr2_b7k_treble":"c-dr-b7k",
    "dr2_b7k_grunt":"c-dr-b7k", "dr2_b7k_attack":"c-dr-b7k",
...
MICPAD = (
    ...
    '<div class="hf-mp-tabs" rata-role="mictabs">'
    '<span class="hf-mp-tab on" data-mic="1" title="Primary mic: the IR\'s own">MIC 1</span>'
    '<span class="hf-mp-tab" data-mic="2" title="Second virtual mic on the same cab">MIC 2</span></div>'
    ...
    '</g></svg>'
    '<div class="hf-mp-m2" rata-role="micm2">%s<div class="hf-mp-m2row">%s%s%s</div></div>'
    '<div class="hf-mp-hidden">%s%s%s%s</div>'
    '</div></div>')

# ── RIG selector (Phase 6, 2026-09-21): a UI-side macro over the whole cab stage —
# cab IR + speaker drive + mic placement + Mic 2 + room + voice + cuts. No port:
# choosing a rig writes the existing per-preset cab ports (set_port_value) and
# clicks the IR picker's built-in option, so presets store the result as they
# always did. The table lives in script-hexforge.js (RIGS); the list is built there.
def rig_sel(scope):
    return ('<div class="hf-ir hf-rig" rata-role="rig" data-scope="%s"><span class="hf-sel-label">Rig</span>'
            '<div class="hf-sel hf-rigsel"><div class="mod-enumerated-selected hf-rigcur" rata-role="rigname">Custom</div>'
            '<div class="hf-riglist" rata-role="riglist"></div></div></div>' % scope)
...
# node-graph style: each block is a node. Click a node to show its controls
...
def amp_body(ns="amp"):
    ...
    rigrow = ('<div class="hf-rigfilter" rata-role="rigfilter" data-scope="%s" title="Show guitar or bass amps">'
              '<button type="button" class="hf-rig-btn hf-rig-on" data-rig="all">ALL</button>'
              '<button type="button" class="hf-rig-btn" data-rig="guitar">GUITAR</button>'
              '<button type="button" class="hf-rig-btn" data-rig="bass">BASS</button>'
              '</div>') % ("rb" if rb else "amp")
    selrow = ('<div class="hf-dselects clearfix">%s%s%s</div>'
              % (render_ctrl(CTRL_BY_SYM["rb_enable"]) if rb else "", rigrow, render_ctrl(CTRL_BY_SYM[S("model")])))
    if rb:
        nam_panel = ('<div class="hf-pa-face hf-nam-face"><div class="hf-pa-title">Neural (NAM)</div>'
                     '<div class="hf-mv-group">' + nam_picker(0, "Amp2Nam", "")
                     + '<div class="hf-onerow">' + render_ctrl(CTRL_BY_SYM["rb_nam_gain"])
                     + render_ctrl(CTRL_BY_SYM["rb_nam_vol"]) + '</div></div></div>')
    else:
        nam_panel = ('<div class="hf-pa-face hf-nam-face"><div class="hf-pa-title">Neural (NAM)</div>'
                     '<div class="hf-mv-group">' + nam_picker(1, "AmpNam", "")
    ...
    face = ('<div class="hf-amp-face hf-face-m1">'
            ...
            + onerow(["gain", "pl_vol2", "pl_variac", "sir34", "evhcomp", "dynload", "master", "sag", "channel", "resonance", "bass", "mid", "treble", "presence"])
            + '</div></div>')
    ...
    if rb:
        blend_panel = ('<div class="hf-pa-face"><div class="hf-pa-title">Blend with Amp 1</div>'
                       + onerow(["blend", "level", "pol", "locut"]) + '</div>')
    ...
    last_tab = ('<div class="hf-atab" rata-role="atab" data-tab="nam" role="tab" tabindex="0" aria-selected="false">Neural</div>'
...
BLOCK_GROUPS = {
    ...
    "dr":  [("DRIVE", None, ["model", "drive", "tone", "level", "mix", "octave"]),
            ("HELSINKI GRIND", "c-dr-b7k", ["b7k_grunt", "b7k_attack", "b7k_bass", "b7k_lomid", "b7k_himid", "b7k_treble"]),
            ("PERFORMANCE", None, ["eco"])],
...
def panel(pfx, title, accent, keys):
    ...
    if pfx == "amp":
        ...
        if pfx == "cab":
            ...
            pad = MICPAD % (render_ctrl(CTRL_BY_SYM["cab_mic2type"]), render_ctrl(CTRL_BY_SYM["cab_mic2lvl"]),
...
def emit_icon():
    ...
    chain = (
        ...
        '    <div class="hf-end hf-in">'
        # IN jack LED doubles as the master MUTE (tuner_mute is a global tail
        # port, independent of the tuner since 2026-08-23): lit = signal flowing.
        '<div class="hf-end-led hf-led-in" mod-role="input-control-port" mod-port-symbol="tuner_mute" mod-widget="switch" role="switch" aria-label="Mute input" title="Mute — lit means signal is flowing"></div>'
        '<span class="hf-end-chip"><span>IN</span></span></div>\n'
        ...
        '    <div class="hf-end hf-out2">'
        # OUT jack LED IS the plugin on/off (mod-role bypass — replaces the old
        # toolbar powerswitch): bypass-light gets .on while the plugin is engaged.
        '<div class="hf-end-led hf-led-out" mod-role="bypass" role="switch" aria-label="Plugin on/off" title="Plugin on / off — lit means engaged"><div class="hf-end-ledcore" mod-role="bypass-light"></div></div>'
        '<span class="hf-end-chip"><span>OUT</span></span></div>\n'
    ...
    return ('<div class="mod-pedal mod-pedal-guitaramp-hexforge{{{cns}}} ">\n'
        ...
        # The × clear button hides itself via CSS :placeholder-shown while empty.
        '          <span class="hf-ps-filter" rata-role="psfilter" title="Show guitar or bass presets">'
        '<button type="button" class="hf-ps-fbtn hf-ps-fon" data-rig="all">ALL</button>'
        '<button type="button" class="hf-ps-fbtn" data-rig="guitar">GUITAR</button>'
        '<button type="button" class="hf-ps-fbtn" data-rig="bass">BASS</button>'
        '</span>'
        '          <span class="hf-ps-searchwrap">'
        ...
        '<button type="button" class="hf-ps-searchclear" rata-role="pssearchclear" aria-label="Clear search" title="Clear search">×</button>'
        '</span>\n'
        ...
        '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M19.14 12.94c.04-.31.06-.62.06-.94s-.02-.63-.06-.94l2.03-1.58a.5.5 0 0 0 .12-.64l-1.92-3.32a.5.5 0 0 0-.61-.22l-2.39.96a7.3 7.3 0 0 0-1.62-.94l-.36-2.54A.5.5 0 0 0 13.9 2h-3.8a.5.5 0 0 0-.49.42l-.36 2.54c-.59.24-1.13.56-1.62.94l-2.39-.96a.5.5 0 0 0-.61.22L2.71 8.48a.5.5 0 0 0 .12.64l2.03 1.58c-.04.31-.06.63-.06.94s.02.63.06.94l-2.03 1.58a.5.5 0 0 0-.12.64l1.92 3.32c.14.24.42.34.61.22l2.39-.96c.49.38 1.03.7 1.62.94l.36 2.54c.04.24.25.42.49.42h3.8c.24 0 .45-.18.49-.42l.36-2.54c.59-.24 1.13-.56 1.62-.94l2.39.96c.24.1.5 0 .61-.22l1.92-3.32a.5.5 0 0 0-.12-.64l-2.03-1.58zM12 15.6a3.6 3.6 0 1 1 0-7.2 3.6 3.6 0 0 1 0 7.2z"/></svg>'
        '<span class="hf-tuner-corner-lab">ADV</span></button>\n'
        '    </div>\n'
        '  </div>\n'
        # ADVANCED panel: FLOW strip directly under the toolbar (2026-08-22 —
        # was an absolute overlay covering the chain controls).
        ...
        '<span class="hf-adv-lab">OUTPUT</span>'
        + render_ctrl(CTRL_BY_SYM["out_auto"])
        + render_ctrl(CTRL_BY_SYM["out_mono"])
        + render_ctrl(CTRL_BY_SYM["out_doubler"])
        + '</div>\n'
        # FRFR row: the voice switch + its four knobs, ALWAYS visible (the whole
        # panel already lives behind the gear — no second layer of hiding).
        ...
        '<span class="hf-adv-lab">FRFR</span>'
        + render_ctrl(CTRL_BY_SYM["out_phase"])
        ...
        # Calibrate button back in the bottom-right corner (2026-08-22, user —
        # it left the ADVANCED panel; the tuner's old corner spot is free).
...
# ── Emit the desktop (JUCE) parameter table ───────────────────────────────────
# One row per control port with a stable id (= the LV2 symbol) and a class the
# JUCE processor uses to decide exposure:
#   HFD_PARAM    preset-captured user parameter  -> APVTS, automatable
#   HFD_SETTING  global user setting (tail port) -> APVTS, non-automatable
#   HFD_COMMAND  pulse/edge command              -> UI bridge only
#   HFD_OUTPUT   plugin -> UI value              -> notify stream only
#   HFD_INTERNAL chain plumbing (slot positions) -> blob/UI bridge only
# Classes are derived, not hand-listed: outputs from out=, commands from the
# fixed command-symbol set, params from the preset-capture range (out_level..
# sw_a, minus clip — mirrors isParamPort in hf_types.inc), hidden params are
# the pos slots (INTERNAL), everything else is a global SETTING.
DESKTOP_COMMAND_SYMS = {
    "sw_a", "sw_b", "sw_c", "sw_d", "ps_bank_up", "ps_bank_dn", "ps_save",
    "ps_move_up", "ps_move_dn", "ps_backup", "ps_restore", "ps_goto", "cal_cmd",
}
DESKTOP_GROUPS = {
    "it": "Input Trim", "gt": "Gate", "cp": "Compressor", "fz": "Fuzz",
    "dr": "Drive", "dr2": "Drive 2", "amp": "Amp", "cab": "Cabinet",
    "md": "Modulation", "dl": "Delay", "rv": "Reverb", "wh": "Wah",
    "oc": "Octave", "nail": "Nail", "eq": "EQ", "rb": "Rig B",
    "out": "Output", "fv": "Output Voice", "tuner": "Tuner",
    "cal": "Calibration", "cpu": "CPU", "ps": "Presets", "sw": "Presets",
    "gt2": "Gate 2", "cp2": "Comp 2", "fz2": "Fuzz 2", "nail2": "Nail 2",
    "md2": "Mod 2", "dl2": "Delay 2", "rv2": "Reverb 2", "wh2": "Wah 2",
    "oc2": "Octave 2", "eq2": "EQ 2",
    "bypass": "Global", "quality": "Global", "clip": "Output",
}
def emit_juce_params():
    def esc(s):
    ...
    def cls(i, c):
        if c["out"]:
            return "HFD_OUTPUT"
        if c["sym"] in DESKTOP_COMMAND_SYMS:
            return "HFD_COMMAND"
        if p0 <= i < p1 and c["sym"] != "clip":
            return "HFD_INTERNAL" if c["hidden"] else "HFD_PARAM"
        return "HFD_SETTING"
    def group(sym):
        return DESKTOP_GROUPS.get(sym.split("_")[0], DESKTOP_GROUPS.get(sym, ""))
    lines = []
    lines.append("// AUTO-GENERATED by build-tools/gen_hexforge.py — do not edit by hand.")
    lines.append("// Desktop (JUCE) parameter table. `port` indexes the HexForgePort enum")
    lines.append("// (lv2/hexforge/hexforge_ports.h); ids are the stable LV2 symbols.")
    lines.append("#pragma once")
    lines.append("")
    lines.append("enum HfDesktopClass : unsigned char {")
    lines.append("    HFD_PARAM = 0, HFD_SETTING = 1, HFD_COMMAND = 2, HFD_OUTPUT = 3, HFD_INTERNAL = 4,")
    lines.append("};")
    lines.append("struct HfScalePoint { const char* label; float value; };")
    lines.append("struct HfDesktopParam {")
    lines.append("    int port; const char* id; const char* name; const char* group;")
    lines.append("    float mn, mx, df;")
    lines.append('    const char* kind;   // "t" toggle | "e" enum | "i" int | "f" float | "db" | "ms" | "hz"')
    lines.append("    HfDesktopClass cls;")
    lines.append("    const HfScalePoint* sp; int nsp;")
    lines.append("};")
    lines.append("")
    for c in ctrl:
        if c["scale"]:
            pts = ", ".join('{ "%s", %s }' % (esc(l), flt(v)) for l, v in c["scale"])
            lines.append("static const HfScalePoint kHfSp_%s[] = { %s };" % (c["sym"], pts))
    lines.append("")
    lines.append("static const HfDesktopParam kHfDesktopParams[] = {")
    for i, c in enumerate(ctrl):
        sp = ("kHfSp_%s" % c["sym"], "int(sizeof(kHfSp_%s)/sizeof(HfScalePoint))" % c["sym"]) \
             if c["scale"] else ("nullptr", "0")
        lines.append('    { %d, "%s", "%s", "%s", %s, %s, %s, "%s", %s, %s, %s },' % (
            NFIXED + i, esc(c["sym"]), esc(c["name"]), esc(group(c["sym"])),
            flt(c["mn"]), flt(c["mx"]), flt(c["df"]), c["kind"], cls(i, c), sp[0], sp[1]))
    lines.append("};")
    lines.append("static const int kHfNumDesktopParams = int(sizeof(kHfDesktopParams)/sizeof(kHfDesktopParams[0]));")
    lines.append("")
    return "\n".join(lines)

# ── Emit the DESKTOP webview page (M2) ────────────────────────────────────────
# Pre-renders the SAME Mustache icon template mod-ui renders at runtime. The
# desktop context: cns/ns empty, the 7 file parameters in mod-ui's order
# (sorted alphabetically by URI), the audio jacks, and EMPTY user-file lists
# (the desktop bridge routes the pickers to a native file dialog instead).
def _mustache(tpl, stack):
    def resolve(name, st):
    ...
    while i < n:
        ...
        if tpl.startswith("{{{", j):
            k = tpl.find("}}}", j)
            v = resolve(tpl[j + 3:k].strip(), stack)
            out.append("" if v is None else str(v)); i = k + 3
        elif tpl.startswith("{{#", j):
            k = tpl.find("}}", j)
            name = tpl[j + 3:k].strip()
            close = "{{/" + name + "}}"
            e = tpl.find(close, k)
            body = tpl[k + 2:e]
            v = resolve(name, stack)
            if isinstance(v, list):
                for item in v:
                    out.append(_mustache(body, stack + [item if isinstance(item, dict) else {}]))
            elif v:
                out.append(_mustache(body, stack + ([v] if isinstance(v, dict) else [])))
            i = e + len(close)
        else:
            k = tpl.find("}}", j)
            v = resolve(tpl[j + 2:k].strip(), stack)
            out.append("" if v is None else str(v)); i = k + 2
    return "".join(out)

DESKTOP_PARAM_URIS = sorted(URI + frag for frag in
    ("#amp2nam", "#ampnam", "#cabnam", "#dr2nam", "#drnam", "#ir2file", "#irfile"))

def emit_desktop_icon(icon_html):
    ctx = {
        "cns": "", "ns": "",
        "effect": {
            "parameters": [{"uri": u, "path": True, "files": []} for u in DESKTOP_PARAM_URIS],
            "ports": {"audio": {
                "input":  [{"name": "In L",  "symbol": "in_l"},
                           {"name": "In R",  "symbol": "in_r"}],
                "output": [{"name": "Out L", "symbol": "out_l"},
                           {"name": "Out R", "symbol": "out_r"}],
            }},
        },
        "files": [], "path": True,
    }
    frag = _mustache(icon_html, [ctx])
    return ("<!doctype html>\n<html>\n<head>\n<meta charset=\"utf-8\">\n"
            "<title>Hex Forge</title>\n"
            "<style>html,body{margin:0;background:#0b0e13;}"
            "body{display:flex;justify-content:center;padding:14px;"
            "font-family:'Segoe UI',system-ui,sans-serif;-webkit-user-select:none;user-select:none;}</style>\n"
            "<link rel=\"stylesheet\" href=\"stylesheet-hexforge-desktop.css\">\n"
            "</head>\n<body>\n"
            + frag +
            "\n<script src=\"vendor/jquery.min.js\"></script>\n"
            "<script src=\"hexforge-ports.js\"></script>\n"
            "<script src=\"hexforge-desktop-bridge.js\"></script>\n"
            "</body>\n</html>\n")

def emit_desktop_css(css_text):
    return css_text.replace("{{{cns}}}", "").replace("{{{ns}}}", "")
...
def emit_ports_js():
    ...
    for c in ctrl:
        if c["out"]:
            continue
        m[c["sym"]] = {"n": c["name"], "k": c["kind"], "mn": c["mn"], "mx": c["mx"],
                       "df": c["df"], "sp": [[l, v] for l, v in c["scale"]] if c["scale"] else None}
    return ("// AUTO-GENERATED by build-tools/gen_hexforge.py — do not edit by hand.\n"
            "// Input control-port metadata for the desktop webview bridge.\n"
            "window.HF_PORTS = " + json.dumps(m, separators=(",", ":")) + ";\n"
            "window.HF_URI = " + json.dumps(URI) + ";\n")
...
if __name__ == "__main__":
    ...
    with open(os.path.join(REPO, "lv2", "modgui-hexforge", "icon-hexforge.html"), "w", newline="\n", encoding="utf-8") as f:
        f.write(icon)
    os.makedirs(os.path.join(REPO, "desktop", "generated"), exist_ok=True)
    with open(os.path.join(REPO, "desktop", "generated", "hexforge_params.h"), "w", newline="\n", encoding="utf-8") as f:
        f.write(jp)
    # Desktop webview page (M2): pre-rendered icon + css + port metadata.
    os.makedirs(os.path.join(REPO, "desktop", "resources"), exist_ok=True)
    with open(os.path.join(REPO, "desktop", "resources", "icon-hexforge-desktop.html"), "w", newline="\n", encoding="utf-8") as f:
        f.write(emit_desktop_icon(icon))
    with open(os.path.join(REPO, "lv2", "modgui-hexforge", "stylesheet-hexforge.css"), "r", encoding="utf-8") as f:
        css = f.read()
    with open(os.path.join(REPO, "desktop", "resources", "stylesheet-hexforge-desktop.css"), "w", newline="\n", encoding="utf-8") as f:
        f.write(emit_desktop_css(css))
    with open(os.path.join(REPO, "desktop", "resources", "hexforge-ports.js"), "w", newline="\n", encoding="utf-8") as f:
        f.write(emit_ports_js())
    ...
    print("      desktop/generated/hexforge_params.h + desktop/resources/{icon,css,ports}")
