...
int main() {
    ...
    {
        const int npOld = HF_N_PORTS - 226;   // v25: 200 param + 26 cpu-region inserts
    ...
    {
        const int npOld = HF_N_PORTS - 223;   // v27: 197 param + 26 cpu
        float vals[HF_N_PORTS];
        for (int i = 0; i < HF_N_PORTS; ++i) vals[i] = 0.0f;
        for (int i = 0; i < npOld; ++i) vals[i] = static_cast<float>(i + 1);
    ...
    {
        const int npOld = HF_N_PORTS - 222;   // v28: 196 param + 26 cpu
        float vals[HF_N_PORTS];
        for (int i = 0; i < HF_N_PORTS; ++i) vals[i] = 0.0f;
        for (int i = 0; i < npOld; ++i) vals[i] = static_cast<float>(i + 1);
    ...
    {
        const int npOld = HF_N_PORTS - 221;   // v29: 195 param + 26 cpu
        float vals[HF_N_PORTS];
        for (int i = 0; i < HF_N_PORTS; ++i) vals[i] = 0.0f;
        for (int i = 0; i < npOld; ++i) vals[i] = static_cast<float>(i + 1);
    ...
    {
        const int npOld = HF_N_PORTS - 247;   // v19: 221 param + 26 cpu
        float vals[HF_N_PORTS];
        for (int i = 0; i < HF_N_PORTS; ++i) vals[i] = 0.0f;
    ...
    {
        const int inserted = 2 + 1 + 5 + 1 + 1 + 6 + 3 + 2 + 2 + 11 + 2 + 1 + 2 + 1 + 1 + 1 + 1 + 16 + 1 + 1 + 10 + 17 + 45 + 1 + 107 + 10 + 2 + 6 + 2 + 2;  // ... + X2 107 + X2 cpu 10 + v39 trims 2 + v40 6 + v41 variac 2 + v42 age 2
    ...
    {
        const int npOld = HF_N_PORTS - 129;   // 119 param (X2 107 + v39 2 + v40 6 + v41 2 + v42 2) + 10 cpu inserts
        float vals[HF_N_PORTS];
        for (int i = 0; i < HF_N_PORTS; ++i) vals[i] = 0.0f;
        for (int i = 0; i < npOld; ++i) vals[i] = static_cast<float>(i + 1);
        ...
        for (int i = HF_SW_A; i < HF_N_PORTS; ++i) {
            ...
            float want = (i >= HF_CPU_GT2 ? i - 129 : i - 119) < npOld
                       ? static_cast<float>((i >= HF_CPU_GT2 ? i - 129 : i - 119) + 1) : 0.0f;
            if (vals[i] != want) {
