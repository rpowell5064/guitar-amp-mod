...
static LV2_Handle hf_instantiate(const LV2_Descriptor*, double rate,
    ...
    if (!hfEngineInit(p, rate)) { delete p->worker; delete p->host; delete p; return nullptr; }
    return p;
}
...
static LV2_State_Status hf_restore(LV2_Handle h, LV2_State_Retrieve_Function retrieve,
    ...
    if (bv && type == p->uris.atom_Chunk && size >= 12) {
        ...
        if (ver < 2 || ver > 48) return LV2_STATE_SUCCESS;    // unknown layout — start fresh (48 = amp_evhcomp lab toggle)
        ...
        const bool needMigrate  = (ver < 48);     // ...v48 amp_evhcomp lab toggle (computed AFTER the np disambiguations)                                 // param-port count at save time
