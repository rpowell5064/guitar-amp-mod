...
static void seedFactoryPresets(HexForge* p) {
    ...
    // reset OFF, then per-preset builds. out_levels measured on-device.
    {
        ...
        {   // B: Brown Sound '84
            ...
            pr.vals[HF_AMP_MODEL] = 10.0f; pr.vals[HF_AMP_GAIN] = 0.92f;
            pr.vals[HF_AMP_PL_VOL2] = 0.35f; pr.vals[HF_AMP_PL_VARIAC] = 1.0f;
            pr.vals[HF_AMP_BASS] = 0.55f; pr.vals[HF_AMP_MID] = 0.6f;
            pr.vals[HF_AMP_TREBLE] = 0.62f; pr.vals[HF_AMP_PRESENCE] = 0.55f;
            pr.vals[HF_AMP_MASTER] = 0.85f; pr.vals[HF_AMP_SAG] = 0.45f;
            pr.vals[HF_MD_ENABLE] = 1.0f; pr.vals[HF_MD_POS] = 3.0f;   // Phase 90 IN FRONT
            pr.vals[HF_MD_TYPE] = 2.0f; pr.vals[HF_MD_RATE] = 0.42f;
            pr.vals[HF_MD_DEPTH] = 0.45f; pr.vals[HF_MD_MIX] = 0.30f;
            pr.vals[HF_DL_ENABLE] = 1.0f; pr.vals[HF_DL_TYPE] = 1.0f;   // tape echo
            pr.vals[HF_DL_TIME] = 310.0f; pr.vals[HF_DL_FEEDBACK] = 0.32f;
            pr.vals[HF_DL_MIX] = 0.18f; pr.vals[HF_DL_WIDTH] = 0.55f;
            ...
            pr.vals[HF_CAB_LOWCUT] = 85.0f; pr.vals[HF_CAB_HIGHCUT] = 8500.0f;
            pr.vals[HF_CAB_MICPOS] = 0.10f; pr.vals[HF_CAB_MICDIST] = 0.10f;
            pr.vals[HF_CAB_ROOMMIX] = 0.12f;
        }
        {   // C: Acca Dacca
            Preset& pr = mk(2, "Acca Dacca");
        ...
        {   // D: Jungle Sleaze
            ...
            pr.vals[HF_GT_THRESH] = -48.0f;
            pr.vals[HF_AMP_MODEL] = 1.0f; pr.vals[HF_AMP_GAIN] = 0.58f;
        ...
        p->presets[17][1].vals[HF_OUT_LEVEL] = -20.1f;
        p->presets[17][2].vals[HF_OUT_LEVEL] = -19.4f;
        p->presets[17][3].vals[HF_OUT_LEVEL] = -17.7f;
    }
...
static LV2_Worker_Status hf_work(LV2_Handle h, LV2_Worker_Respond_Function respond,
    ...
    if (msg->type == W_CAB_IR) {
        ...
        if (msg->namSlot == 1) {   // Rig B cab: built-in sentinel OR a user .wav (v39)
            if (msg->path[0] == '@')
                p->cab2.setIR(CabModels::generate(msg->path, p->rate));
