function (event, funcs) {
    ...
    function applyAmp(icon) {
        ...
        show(icon, 'amp', '.c-amp-comp', m === 1 || m === 2 || m === 4 || m === 6 || m === 8 || m === 10 || m === 11 || m === 12 || m === 14);
    ...
    function setIr2Label(icon) {
        ...
        if (CAB_NAMES[v]) { setFile(icon, 'Ir2', v, CAB_NAMES[v]); panelOf(icon, 'cab2').removeClass('hf-ir-user'); return; }
        setFile(icon, 'Ir2', v, 'Factory Cab (built-in)');
        panelOf(icon, 'cab2').toggleClass('hf-ir-user', v !== '@builtin');   // a user .wav on Cab 2: the basics only
        if (v !== '@builtin') cabTabSet(icon, 'cab2', 'cab');
    }
    ...
    function setIr(icon, value) {
        ...
        if (CAB_NAMES[value]) {                      // built-in synthetic cab
            ...
            panelOf(icon, 'cab').removeClass('hf-ir-user');
            return;
        }
        setFile(icon, 'Ir', value, null);            // user .wav → basename
        setNodeVal(icon, 'cab', icon.find('[rata-role=Ir]').first().text());
        // A user .wav gets the basics only: the rigs and MIC & ROOM are for the built-in cabs (user 2026-09-24).
        panelOf(icon, 'cab').addClass('hf-ir-user');
        cabTabSet(icon, 'cab', 'cab');
    }
